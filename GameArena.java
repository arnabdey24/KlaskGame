...
public class GameArena extends JPanel implements Runnable, KeyListener, MouseListener, MouseMotionListener {
        ...
        synchronized (this) {
            if (!this.exiting) {
                ...
                for (Object o : things) {
                    ...
                    if (o instanceof ServeArea) {
                        ServeArea s = (ServeArea) o;
                        graphics.setColor(this.getColourFromString(s.getColour()));
                        graphics.setStroke(new BasicStroke((float) s.getThickness()));
                        graphics.draw(new Arc2D.Double(s.getX(), s.getY(), s.getW(), s.getH(), s.getStart(), s.getExtent(), Arc2D.OPEN));
                    }

                    if (o instanceof Bat) {
                        Bat b = (Bat) o;
                        graphics.setColor(this.getColourFromString(b.getColour()));
                        graphics.draw(new Line2D.Float((float) b.getXPosition(), (float) b.getYPosition(),
                                (float) b.getXPosition(), (float) (b.getYPosition() - b.getSize())));
                        graphics.fillOval((int) (b.getXPosition() - b.getSize() / 2), (int) (b.getYPosition() - b.getSize() / 2), (int) b.getSize(), (int) b.getSize());
                    }
     ...
     * Once a Text object is added, it will automatically appear on the window.
     *
     * @param s the text object to add to the GameArena.
     */
    public void addSemiCircle(ServeArea s) {
        this.addThing(s, 0);
    }
     ...
     * Once a Text object is added, it will automatically appear on the window.
     *
     * @param b the text object to add to the GameArena.
     */
